						...
						if (rawResponse != null) {
							...
							if (crypto == null) {
								if (containsAndreError(response) || containsHTML(response)) {
									if (!containsAndreError(response) && !containsHTML(response)) idx++;
									else Log.i(TAG, "Invalid response:" + new String(response, 0, response.length));
								}
							} else {
								if (!containsROMessage(response) && !containsAndreError(response) && !containsHTML(response)) {
									response = crypto.decryptData(response);
									if (containsAndreError(response) || containsHTML(response)) Log.i(TAG, "Invalid response(cluster):" + new String(response, 0, response.length));
								} else if (containsAndreError(response) || containsHTML(response)) Log.i(TAG, "Invalid response(extern):" + new String(response, 0, response.length));
							}
							connected = true;
							if (Global.doMediusCallToServer || (didServerCall == false || connected != false) && (!UseCaching || cache == null)) {
								...
								response = null;
								if (cache != null) {
									if (!containsROMessage(cache)) response = crypto.decryptData(cache);
									else response = cache;
								}
							}
				...
				if (rawResponse != null) {
					...
					if (crypto != null) {
						if (!containsROMessage(result) && !containsAndreError(result) && !containsHTML(result)) {
							...
							if (containsAndreError || containsHTML(result)) Log.e(TAG, "Invalid response(cluster):" + new String(result, 0, result.length));
